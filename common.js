...
var GlobalSt = {"VMFuncEn" : false, "waitTimeScreen" : false , "HideModemMode" : false, "title": "Connect Box" ,"goto":false, "HideRemoteAccess":false};
//Token mechanism S
var _ajaxWorking = 0;
var _ajaxLine = [];
...
IPV6_SINGLE_ADDRESS_REGULAR_EXPRESSION = /^([0-9a-fA-F]{1,4})$/;
...
function IPv6SingleAddrTest(addr)
{
	return (IPV6_SINGLE_ADDRESS_REGULAR_EXPRESSION.test(addr));
}
...
function changePWstatus(status, ArrowPosition, textPosition){
    ...
    switch(status){    
        case 'weak':
            ...
            $('#' + textPosition + ' span').html(getLanguageResourcesById("c_st18")).attr("class","red");
        break;
        case 'good':
        ...
        case 'extrastrong':
            ...
            $('#' + textPosition + ' span').html(getLanguageResourcesById("c_st22")).attr("class","green");
        break;
    }
...
function isVaildUserPwd(pwd)
{
	...
	var vaild = false;
	if(Upper>0 && Number > 0 && lower > 0 && pwd.length >= 8 && pwd.length < 32){
		vaild = true;
	}
	return vaild;
}
